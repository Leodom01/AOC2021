...
# Fine only if I have only vertical, horizontal or 45 degrees line
for seg in segments:
    ...
    # If line is \
    elif seg.direction.unit.x == seg.direction.unit.y:

        y_coord = min(seg.p1.y, seg.p2.y)
        for x_val in range(min(seg.p1.x, seg.p2.x), max(seg.p1.x, seg.p2.x) + 1):
            density_matrix[y_coord][x_val] += 1
            y_coord += 1
    # If line is /
    elif seg.direction.unit.x != seg.direction.unit.y:
        y_coord = max(seg.p1.y, seg.p2.y)
        for x_val in range(min(seg.p1.x, seg.p2.x), max(seg.p1.x, seg.p2.x) + 1):
            density_matrix[y_coord][x_val] += 1
            y_coord -= 1
